...
// Définition initiale du nombre de produits et de promotions dans le panier
$count = new stdClass();
$count->nbre = 0;
...
if (isset($_SESSION['user']) && isset($_SESSION['user']->id_client)) {
    ...
    try {
        // Préparez la requête SQL pour compter le nombre total de produits et de promotions dans les paniers
        $itemCountQuery = "SELECT COUNT(*) AS nbre 
                           FROM paniers p
                           JOIN panier_produits pp ON p.id_panier = pp.id_panier
                           WHERE p.id_client = :userId";
        $execut_panier = $connexion->prepare($itemCountQuery);
   ...
   <div class="delivery-banner bg-beige text-center py-2">
    <p class="m-0">
        ...
        <?php if (isset($_SESSION['user'])): ?>
            &nbsp;<?= htmlspecialchars($_SESSION['user']->prenomc); ?> <?= htmlspecialchars($_SESSION['user']->nomc); ?>
        <?php endif; ?> 
        &nbsp; &nbsp;
        <a class="nav-link text-white category-card" href="logout.php"><i class="fas fa-sign-out-alt"></i>&nbspDÉCONNEXION</a>
    </p>
</div>
...
<section class="bg-pink py-5">
    <div class="container">
        <div class="row row-cols-2 row-cols-md-4 g-4">
         ...
         <a href="produits.php" class="btn btn-success offset-5 mt-4">TOUT AFFICHER</a>
    </div>
</section>
...
<section class="instagram-section py-5 d-md-none">
  ...
  <div id="whatsappButton">
    <a href="https://api.whatsapp.com/send?phone=221781326969" target="_blank" class="btn btn-success btn-lg"><i class="fab fa-whatsapp"></i> </a>
  </div>
...
<script>
  var swiper = new Swiper('.swiper-container', {
    ...
    autoplay: {
      delay: 3000, // Définit le délai entre chaque slide (3000 ms = 3 secondes)
